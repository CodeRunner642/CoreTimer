...
export function App() {
  ...
  if (view === 'timer') {
    ...
    return <div className="screen calm timer-screen"><div className="timer-layout"><div className="timer-head"><div className="kicker">SET {current?.set || 1} OF {config.sets}</div><h1 className="hero timer-hero">{current?.label}<span>.</span></h1></div><div className="timer-bubble-wrap" style={{ '--ring-progress': phaseProgress }}><div className="timer-ring" /><div className="circle warm timer-bubble">{Math.ceil(remainingMs / 1000)}</div></div><h2 className="accent center">{current?.label}.</h2><p className="timer-meta">Rep {current?.rep || 1} of {config.reps}</p><p className="timer-next">Next: {next ? `${next.label} (${next.duration}s)` : 'Complete'}</p><div className="progress"><div style={{ width: `${clamp((doneMs / (totalMs || 1)) * 100, 0, 100)}%` }} /></div><div className="row"><span>{formatTime(doneMs)}</span><span>{formatTime(totalMs)}</span></div><div className="timer-buttons"><button onClick={() => running ? setRunning(false) : (phaseEndRef.current = performance.now() + remainingMs, setRunning(true))}>{running ? 'Pause' : 'Resume'}</button><button className="ghost" onClick={() => resetSession('home')}>End session</button></div></div></div>;
  }
