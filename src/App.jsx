...
import { buildConfidenceDiagnostics, stage2DiagnosticsData } from './stage2Diagnostics';
...
export function App() {
  ...
  const stage2Diagnostics = useMemo(() => buildConfidenceDiagnostics(stage2DiagnosticsData, 5), []);
    const greeting = getGreeting();
  const name = data.displayName?.trim();
  return <div className="screen calm"><div className="kicker">{new Date().toLocaleDateString(undefined, { weekday: 'long', month: 'short', day: 'numeric' }).toUpperCase()}</div><h1 className="hero">{greeting}{name ? ',' : ''}<br />{name ? <span>{name}.</span> : <span>there.</span>}</h1><div className="card"><div className="row"><div className="kicker">TODAY'S SESSION</div><strong>{formatTime(totalMs)}</strong></div><div className="session"><div className="circle warm small" /><div><h2>{level.label}</h2><p>{data.includedExercises.length} groups · prep {data.prepareSeconds}s</p></div></div><div className="session-tracker" aria-label="Today's completed sessions">{Array.from({ length: sessionsPerDayTarget }).map((_, idx) => <span key={idx} className={`session-dot ${idx < todaySessions ? 'done' : ''}`} aria-hidden="true">{idx < todaySessions ? '✓' : ''}</span>)}</div><p className="session-progress-label">{todaySessions} of {sessionsPerDayTarget} sessions completed today</p><button data-testid="start-session" onClick={startSession}>{todayDone ? 'Start again' : 'Begin'} →</button></div><div className="card"><div className="row"><div className="kicker">CONSISTENCY</div><strong>BEST {data.bestStreak}</strong></div><h2>{data.currentStreak} {data.currentStreak === 1 ? 'day' : 'days'}</h2></div><div className="card"><div className="kicker">STAGE 2 CONFIDENCE DIAGNOSTICS</div><div className="row"><span>High/Medium/Low/Uncertain</span><strong>{stage2Diagnostics.summary.highConfidenceCount}/{stage2Diagnostics.summary.mediumConfidenceCount}/{stage2Diagnostics.summary.lowConfidenceCount}/{stage2Diagnostics.summary.uncertainCount}</strong></div><div className="row"><span>Score distribution</span><strong>{Object.entries(stage2Diagnostics.scoreDistribution).map(([k,v]) => `${k}: ${v}`).join(' · ')}</strong></div><p className="subtle">Data completeness blockers: {Object.entries(stage2Diagnostics.reasonBreakdownByCategory.dataCompletenessBlockers).map(([reason, count]) => `${reason} (${count})`).join(', ') || "none"}</p><p className="subtle">Confidence quality blockers: {Object.entries(stage2Diagnostics.reasonBreakdownByCategory.confidenceQualityBlockers).map(([reason, count]) => `${reason} (${count})`).join(', ') || "none"}</p><p className="subtle">Activation blockers: {Object.entries(stage2Diagnostics.reasonBreakdownByCategory.activationBlockers).map(([reason, count]) => `${reason} (${count})`).join(', ') || "none"}</p><p className="subtle">Next recommended action: <strong>{stage2Diagnostics.nextRecommendedAction}</strong></p><p><a href="/api/debug/stage2/confidence-diagnostics" target="_blank" rel="noreferrer">Open confidence diagnostics JSON</a></p><p className="subtle">Stage 2 predictions enabled: false · activationReady: false</p></div><nav className="tabbar"><button className="active">Today</button><button onClick={() => setView('history')}>History</button><button onClick={() => setView('settings')}>Settings</button></nav></div>;
}
