...
export function App() {
  ...
  if (view === 'timer') {
    ...
    return <div className="screen calm"><div className="kicker">SET {current?.set || 1} OF {config.sets}</div><h1 className="hero">{current?.label}<span>.</span></h1><div className="circle-wrap big"><div className="circle warm">{Math.ceil(remainingMs / 1000)}</div></div><h2 className="accent">{current?.label}.</h2><p className="subtle">Rep {current?.rep || 1} of {config.reps}</p><div className="progress"><div style={{ width: `${clamp((doneMs / (totalMs || 1)) * 100, 0, 100)}%` }} /></div><div className="row"><span>{formatTime(doneMs)}</span><span>{formatTime(totalMs)}</span></div><button onClick={() => running ? setRunning(false) : (phaseEndRef.current = performance.now() + remainingMs, setRunning(true))}>{running ? 'Pause' : 'Resume'}</button><button className="ghost" onClick={() => resetSession('home')}>End session</button></div>;
  }
